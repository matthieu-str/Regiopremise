...
class Regiopremise:
    def __init__(self, bw_project_name, premise_database_name, ecoinvent_database_name, ecoinvent_version):
        ...
        # set up logging tool
        self.logger = logging.getLogger("Regiopremise")
        ...
        if ecoinvent_version not in ["3.9", "3.9.1", "3.10", "3.10.1"]:
            raise KeyError(
                "The version of ecoinvent you provided is not supported by Regiopremise."
        ...
        with as_file(files('regioinvent').joinpath(
                f"data/Regionalization/ei_premise_iam_locations_geo_mapping.json")) as file_path:
            with open(file_path, "r") as f:
    ...
    def spatialize_my_premise(self):
        ...
        if self.name_ei_with_regionalized_biosphere not in bw2.databases:
            ...
            self.ei_wurst = wurst.extract_brightway2_databases(
                self.premise_database_name, add_identifiers=True
            )
            ...
            self.logger.info(
                "There is already a spatialized version of premise in your project. If you want to redo "
    ...
    def first_order_regionalization(self):
        ...
        self.logger.info(
            "Regionalizing main inputs of internationally-traded products of premise..."
        )
                ...
                for producer in producers.index:
                    ...
                    if not regio_process:
                        if "World" in possibilities[technology]:
                            regio_process = copy_process(
                                product, technology, "World", producer
                            )
                    ...
                    while good_regio_process is False:
                        ...
                        if len(technosphere_flows) == 1:
                            if product == technosphere_flows[0]['product']:
                                ...
                                self.skipped_regio_processes.append(regio_process)
                                global_market_activity['exchanges'] = [
                                ...
                                regio_process = copy_process(product, technology, next_loc, producer)
                            else:
    ...
    def spatialize_elem_flows(self):
        """
        Function spatializes the elementary flows of the regiopremise processes to the location of process.
        """

        self.logger.info(
            "Regionalizing the elementary flows of the regiopremise database..."
        )
    ...
    def connect_ecoinvent_to_regioinvent(self):
        ...
        self.logger.info("Connecting premise to regioinvent processes...")
